...
async function compareUrls(urlA, urlB, opts = {}) {
  ...
  const framesA = await captureFrames(
    browser,
    urlA,
    labelA,
    outDir,
  ...
  const framesB = await captureFrames(
    browser,
    urlB,
    labelB,
    outDir,
  ...
  for (let i = 0; i < count; i++) {
    ...
    const diffFile = path.join(
      outDir,
      `diff_frame_${i}_scroll_${fa.scrollY}.png`
    );
    const result = await pixelDiff(fa.filename, fb.filename, diffFile, {
      threshold,
    });
    ...
    log(
      `Frame ${i} (scrollY=${fa.scrollY}): ${status} — ${result.pct}% pixels differ${methodNote}`
    );
  ...
  if (saveReport) {
    ...
    fs.writeFileSync(
      reportFile,
      JSON.stringify({ urlA, urlB, frames, summary }, null, 2)
    );
